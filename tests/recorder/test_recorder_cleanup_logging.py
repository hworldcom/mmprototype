...
def test_cleanup_logs_close_errors(tmp_path, monkeypatch, caplog):
    ...
    monkeypatch.setattr(recorder_mod, "BufferedCSVWriter", _BadWriter)
    ...
    assert any("Failed to close orderbook_writer" in rec.message for rec in caplog.records)
